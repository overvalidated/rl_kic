...
def prepare_env(
    ...
    cdef int shift_prep = 8#*np.random.randint(2, 7)
    chosen_modes = []
    shifts = []
    for person_ in range(n_persons):
        chosen_modes += [np.random.choice([22, 33, 43, 52])]
        if chosen_modes[-1] == 22:
            ...
            shifts += [np.random.randint(-1, 2)]
    shifts = [0,0,2,2]
    for n in range(shift_prep):
        if n % 2 == 0:
            for mode, shift_ in zip(chosen_modes, shifts):
                if check_mode(n, 22, shift_):
                    state, _ = next_state(**state, action=4)
                else:
